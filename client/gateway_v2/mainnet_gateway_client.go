...
func (c *MainnetGatewayClient) WithdrawMintableERC721(caller *client.Identity, amount *big.Int, tokenAddr common.Address, sigs []byte, validators []common.Address) error {
	hash := c.withdrawalHash(caller.MainnetAddr, tokenAddr, tgtypes.TransferGatewayTokenKind_ERC20, big.NewInt(0), amount)
	v, r, s, valIndexes, err := client.ParseSigs(sigs, hash, validators)
	if err != nil {
		return err
	...
	tx, err := c.contract.WithdrawMintableERC721(client.DefaultTransactOptsForIdentity(caller), amount, tokenAddr, valIndexes, v, r, s)
	if err != nil {
		return err
	}
	return client.WaitForTxConfirmation(context.TODO(), c.ethClient, tx, c.TxTimeout)
