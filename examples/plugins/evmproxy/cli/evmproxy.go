...
func main() {
	...
	getCmd := &cobra.Command{
		...
		RunE: func(cmd *cobra.Command, args []string) error {
			result, err := GetValueCmd(flags.ChainId, flags.WriteUri, flags.ReadUri, flags.ContractHexAddr, flags.KeyType)
			if err == nil {
	...
	setCmd := &cobra.Command{
		...
		RunE: func(cmd *cobra.Command, args []string) error {
			return SetValueCmd(flags.ChainId, flags.WriteUri, flags.ReadUri, flags.ContractHexAddr, flags.KeyType, value)
		},
	}
...
func GetValueCmd(chainId, writeUri, readUri, contractHexAddr, keyType string) (string, error) {
...
func SetValueCmd(chainId, writeUri, readUri, contractHexAddr, keyType string, value int) error {
	rpcClient := client.NewDAppChainRPCClient(chainId, writeUri, readUri)
	contractAddr, err := loom.LocalAddressFromHexString(contractHexAddr)
	if err != nil {
		return err
